...
from transformers import VideoMAEFeatureExtractor, VivitModel
...
#huggingface_hub.login() #hugging face login
# 실제 카메라 FPS 가져오기
...
out = cv2.VideoWriter(
    output_video_path, fourcc, 10, (frame_width, frame_height)
)
...
while recording and not stop:
    ...
    if stop:
        ...
        print(output_video_path)
        outputs = video_cls(output_video_path)
    
        st.write(f"Predicted label: {outputs[0]}")
